...
import { fetchAllOrdersWithAddressAdmin } from '../api';
...
const AdminOrdersPage = () => {
    ...
    // 2. СПРОЩУЄМО функцію завантаження даних до одного запиту
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetchAllOrdersWithAddressAdmin();
    ...
    // Допоміжна функція для форматування дати - без змін
                                ...
                                <Card.Body>
                                    ...
                                    {order.username ? (
                                        <>
                                            ...
                                            {order.phone && <p className="mb-0"><strong>Телефон:</strong> {order.phone}</p>}
                                        </>
                                    ) : (
                                        <p className="text-muted">Інформація відсутня</p>
                                    )}
                                    ...
                                    {order.address && order.address.city ? (
                                        <div>
                                            <small>
                                                {order.address.country}, {order.address.postalCode && `${order.address.postalCode}, `}{order.address.region && `${order.address.region}, `}{order.address.city}, {order.address.street} {order.address.houseNumber}
